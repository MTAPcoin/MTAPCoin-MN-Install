...
* First, we will do the initial collateral TX and send exactly 2000 MTAP to one of our addresses. To keep things sorted in case we setup more masternodes we will label the addresses we use.

  - Open your MTAP wallet and switch to the "Receive" tab.
  ...
  - Once all addresses are created send 2000 MTAP each to them. Ensure that you send exactly 2000 MTAP and do it in a single transaction. You can double check where the coins are coming from by checking it via coin control usually, that's not an issue.
...
Close your wallet and open the MTAP Appdata folder. Its location depends on your OS.

* **Windows:** Press Windows+R and write %appdata% - there, open the folder MTAP.  
* **macOS:** Press Command+Space to open Spotlight, write ~/Library/Application Support/MTAP and press Enter.  
* **Linux:** Open ~/.MTAP/

In your appdata folder, open masternode.conf with a text editor and add a new line in this format to the bottom of the file:

```bash
masternodename ipaddress:6987 genkey collateralTxID outputID
```

An example would be

```
mn1 127.0.0.2:6987 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0
```
...
```bash
bash <( curl https://raw.githubusercontent.com/MTAPcoin/MTAPCoin-MN-Install/master/install.sh )
```
...
**PLEASE NOTE**: Do not choose the advanced installation option unless you have experience with Linux and know what you are doing - if you do and something goes wrong, the MTAP team CANNOT help you, and you will have to restart the installation.
